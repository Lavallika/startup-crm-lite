...
const RecentLeads = ({ leads }) => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${styles[status] || 'bg-slate-100 text-slate-700 dark:bg-gray-700 dark:text-gray-300'}`}>
        {status}
      </span>
    );
  };
              ...
              recentLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors">
                  <td className="py-3 px-4 md:px-6">
                    <div className="font-medium text-sm text-slate-800 dark:text-gray-200 truncate max-w-[120px] md:max-w-none">
                      {lead.name}
                    </div>
                  </td>
                  <td className="py-3 px-4 md:px-6 text-sm text-slate-600 dark:text-gray-400 hidden sm:table-cell truncate max-w-[140px] md:max-w-none">
                    {lead.company}
                  </td>
                  <td className="py-3 px-4 md:px-6">
                    {getStatusBadge(lead.status)}
                  </td>
                  <td className="py-3 px-4 md:px-6 text-sm text-slate-600 dark:text-gray-400 hidden md:table-cell whitespace-nowrap">
                    {lead.dateAdded}
                  </td>
                </tr>
              ))
